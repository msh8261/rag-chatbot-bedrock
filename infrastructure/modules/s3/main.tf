# S3 Module - Secure RAG Chatbot S3 Buckets
...
# S3 Bucket Notification for Documents (for Bedrock Knowledge Base)
...
# S3 Bucket Policy for Documents
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Effect = "Allow"
        Principal = {
          AWS = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.project_name}-${var.environment}-bedrock-kb-role"
        }
        Action = [
          "s3:GetObject",
          "s3:ListBucket"
